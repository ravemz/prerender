...
client.on('ready', function () {
    redisOnline = true;
    util.log('Redis Cache Connected');
});
...
module.exports = {
    requestReceived: function (req, res, next) {
        ...
        var redisKey = keyPrefix + req.prerender.url;
        ...
        client.get(redisKey, function (error, result) {
            if (!error && result) {
                ...
                util.log('Redis Cache Hit: ' + redisKey);
    ...
    pageLoaded: function (req, res, next) {
        ...
        var key = keyPrefix + req.prerender.url;
        var response = {
        ...
        client.set(key, JSON.stringify(response), function (error, reply) {
            ...
            if (!error && reply && TTL) {
                contentSize = Math.ceil(response.content.length/1024);
                util.log('Redis cache of size ' + contentSize + 'KB added: ' + key);

                if(TTL > 0) {
                  // set expiry only if its set, else never expire
